...
/**
 * Función pura cacheable. Recibe userId como argumento (sin auth() interno)
 * para que unstable_cache pueda generar una clave por usuario.
 * NO debe llamarse directamente desde fuera — usar getMetricasDia().
 */
async function calcularMetricasDia(userId: string): Promise<MetricasDia> {
...
/**
 * Versión cacheada de calcularMetricasDia.
 * Cache key: userId (automático por argumento) + "metricas-dia" (key part).
 * Revalida cada 30s. Se invalida manualmente con revalidateTag("metricas-dia")
 * desde las acciones de registro/edición/eliminación.
 */
const obtenerMetricasDiaCached = unstable_cache(
  calcularMetricasDia,
  ["metricas-dia"],
  { revalidate: 30, tags: ["metricas-dia"] },
);
...
export async function getMetricasDia(): Promise<MetricasDia | null> {
  try {
    ...
    return await obtenerMetricasDiaCached(userId);
  } catch (e) {
