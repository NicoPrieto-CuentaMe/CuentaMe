  ...
  try {
    ...
    for (let i = 0; i < parsed.length; i++) {
      ...
      if (!platoId) {
        return {
          ok: false,
          message: lineaMsg(i + 1, "falta el plato."),
          field: `linea-${i}`,
        };
      }
    ...
    if (unique.size !== platoIds.length) {
      return {
        ok: false,
        message: "No puedes repetir el mismo plato en dos líneas.",
        field: "lineas",
      };
    }
    ...
    for (let i = 0; i < parsed.length; i++) {
      ...
      if (!plato) {
        return {
          ok: false,
          message: lineaMsg(lineNum, "el plato no es válido o no está activo."),
          field: `linea-${i}`,
        };
      }
